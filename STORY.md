...
### random projects
after "giving up" on fantasummer and sticky (which should lowkey be considered part of these random projects), i decided to work on a couple random projects to build experience and solve stupid problems i have. first was tunepilot, a local music player with discord integration. imma be honest i built this specifically so my friends could see the unreleased music i listen to. i based it on a design that i made in figma like a year earlier, but i never finished it because working with a lot of files is boring, and there's just no good discord javascript apis. then i designed mezzoparty (dumb name ik), a react native app (my first react native project that made it past the initial template) consisting of a bunch of games that you can play with your friends while being together physically. stuff like impostor, truth or dare, and more. i never finished it because i probably need a game engine for some stuff and idk how to work that. i might work on it some more one day.

### berto.life
around the luverz and fantaszn era i made a little portfolio website to host my work, however i never used it and it was pretty stupid. but one day, i decided to buy the domain berto.life for like 2 euros and i remade that portfolio and then i changed the whole direction of it and swapped that freshly remade portfolio (which is now [here](old.berto.life)) with a new simpler one (which you can find [here](berto.life)).

### matcha
what if there was a dating app for minors? YES THAT SOUNDS LIKE THE WORST IDEA OF ALL TIME BUT HEAR ME OUT. only available for high school students (13-19 years old) with algorithms to specifically prevent 19 year olds matching with 13 year olds. students will have to verify in one way or another, to make sure that they are actually students and not 50 year olds. in a hypothetical scenario, the app would function basically exactly like hinge, with various prompts that people can respond to in order to start a conversation which has a 3 day timer (the idea is that you have to get to know the other person quickly and get their number or instagram). hopefully one day i'll finish it. i also have a working react native prototype, auth and a simple backend
